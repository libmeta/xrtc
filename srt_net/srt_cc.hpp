...
public:
    ...
    bool updateVideoEncodeBitate(int64_t& current_vencode_bitrate, const int64_t video_stream_bitrate)
    {
        ...
        if (update_vencode_bitrate_task.doByTimeAndCount([this, &current_vencode_bitrate, &video_stream_bitrate, &tmp_vencode_bitrate] {
    ...
    void congestionCtrl()
    {
        if (srt_congestion_ctrl_task.doByTimeAndCount([this] {
...
private:
    ...
    RunTask update_vencode_bitrate_task { VIDEO_UPDATE_INTERVAL };
    RunTask srt_congestion_ctrl_task { SRT_CHECK_INTERVAL };
};
